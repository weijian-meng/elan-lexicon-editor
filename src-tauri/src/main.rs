...
use std::sync::Mutex;
use tauri::State;
use xml_utils::{parse_xml, build_xml};
...
mod diff_utils;
use diff_utils::{diff_lexicons, resolve_source, Source, DiffOptions};

#[tauri::command]
async fn diff(left: Source, right: Source, options: DiffOptions) -> Result<serde_json::Value, String> {
    diff_lexicons(resolve_source(&left).map_err(|e| e.to_string())?, resolve_source(&right).map_err(|e| e.to_string())?, options)
}
...
fn main() {
    tauri::Builder::default()
        .manage(AppState { modified: Mutex::new(false) })
