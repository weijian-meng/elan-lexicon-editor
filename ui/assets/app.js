...
function setIsModified(value) {
    ...
    try {
        ...
        if (invoke) {
            invoke('set_modified', { modified: isModified });
            console.log(`Successfully sent modified state to Python: ${isModified}`);
        } else {
            console.log('Tauri API not available for set_modified');
        }
    } catch (e) {
...
openFileBtn.addEventListener('click', function () {
    ...
    alert('Open button clicked! Checks: ' + (window.__TAURI__ ? 'Tauri OK' : 'No Tauri'));
    handleOpenFile();
});
...
async function handleOpenFile() {
    ...
    try {
        ...
        console.log('Calling open_file API using:', invoke);
        ...
        const parsedXml = await invoke('parse_xml_command', { content: content });
...
async function handleSaveFile() {
    ...
    try {
        if (!currentFile) {
            if (!window.__TAURI__) {
                console.log('Tauri API not ready');
                alert('The native bridge is not ready yet. Please try again.');
                return false;
            }
            console.log('Getting save file dialog...');
            const invoke = (window.__TAURI__.core && window.__TAURI__.core.invoke) ? window.__TAURI__.core.invoke : window.__TAURI__.invoke;
            const result = await invoke('save_file_dialog_command');
            if (result) {
                currentFile = result;
        ...
        if (currentFile && lexicon) {
            ...
            const invoke = (window.__TAURI__.core && window.__TAURI__.core.invoke) ? window.__TAURI__.core.invoke : window.__TAURI__.invoke;
            const xmlString = await invoke('build_xml_command', { data: { lexicon: lexiconToSave } });
            console.log('Saving file...');
            const saveResult = await invoke('save_file_command', { filePath: currentFile, content: xmlString });
